...
export class FoodDatabaseService {
    ...
    try {
      ...
      try {
        ...
        if (rows && rows.length > 0) {
          ...
          const newFavoriteStatus = !existingPref.is_favorite;
          
          // Update using direct SQL
          await db.$client.query(
            `UPDATE user_food_preferences 
             SET is_favorite = $1, last_viewed = $2 
             WHERE id = $3`,
          ...
          await db.$client.query(
            `INSERT INTO user_food_preferences 
             (user_id, food_item_id, is_favorite, view_count, last_viewed) 
             VALUES ($1, $2, $3, $4, $5)`,
  ...
  async getUserFavourites(userId: number): Promise<FoodItem[]> {
    try {
      ...
      try {
        // Use correct SQL column name is_favorite (American spelling)
        const { rows } = await db.$client.query(
          `SELECT * FROM user_food_preferences WHERE user_id = $1 AND is_favorite = true`,
          [userId]
        );
