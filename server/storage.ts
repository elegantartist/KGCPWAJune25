...
export interface IStorage {
  ...
  getUserFavourites(userId: number, contentType: string): Promise<UserFavorite[]>;
...
export class MemStorage implements IStorage {
  ...
  constructor() {
    ...
    metrics.forEach((metric, index) => {
      const healthMetric: HealthMetric = {
        ...metric,
        id: this.metricCurrentId++,
        date: dates[index]
      };
      this.healthMetrics.set(healthMetric.id, healthMetric);
    });
    
    // Create default Care Plan Directives (CPDs)
    const now = new Date();
    const defaultCPDs: Array<Omit<CarePlanDirective, 'id'>> = [
      {
        userId: defaultUser.id,
        directive: "Eat more fruit and vegetables and smaller portions",
        category: "diet",
        targetValue: 8,
        active: true,
        createdAt: now,
      ...
      {
        userId: defaultUser.id,
        directive: "Try to raise your heart rate above 120 bpm for 30 minutes 3 times per week",
        category: "exercise",
        targetValue: 8,
        active: true,
        createdAt: now,
      ...
      {
        userId: defaultUser.id,
        directive: "Take your Ozempic once per week",
        category: "medication",
        targetValue: 10,
        active: true,
        createdAt: now,
    ...
    // Add the CPDs to the map
    defaultCPDs.forEach(cpd => {
      const directive: CarePlanDirective = {
        ...cpd,
        id: this.directiveCurrentId++
      };
      this.carePlanDirectives.set(directive.id, directive);
    });
  }
  ...
  async getRecentRecommendations(userId: number, limit: number = 5): Promise<Recommendation[]> {
    return this.getRecommendations(userId).slice(0, limit);
  }
