...
function generateUIN(prefix: string, doctorId?: number): string {
  // Just generate a simple UIN with prefix and random digits
  // This simplifies the function while optimization is in progress
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Create a new health metric for a specific user
  ...
  app.get("/api/users/:userId/favorite-videos", async (req, res) => {
    try {
      ...
      // Query database for favorite videos
      const favoriteVideos = await db
        .select()
        .from(userFavoriteVideos)
        .where(eq(userFavoriteVideos.userId, userId))
  ...
  // Search for cooking videos on YouTube with enhanced Tavily configuration
  app.post("/api/recipes/videos", async (req, res) => {
    try {
      ...
      // Get user's active Care Plan Directives for enhanced search
      let cpdDietaryTerms: string[] = [];
      let userAllergies: string[] = [];
      let userDietaryRestrictions: Record<string, boolean> = {};
      
      // Check if the front-end included the doctor's Care Plan Directive directly
      let doctorCPD = '';
      ...
      try {
        ...
        if (dietCPDs.length > 0) {
          // Extract potential dietary terms from the directive text
          for (const dietCPD of dietCPDs) {
            const directive = dietCPD.directive.toLowerCase();
            
            // Extract specific diet-related terms
            if (directive.includes('low carb') || directive.includes('reduce carb')) {
              cpdDietaryTerms.push('low-carb');
              userDietaryRestrictions['low-carb'] = true;
            }
            if (directive.includes('low fat') || directive.includes('reduce fat')) {
              cpdDietaryTerms.push('low-fat');
              userDietaryRestrictions['low-fat'] = true;
            }
            if (directive.includes('low sodium') || directive.includes('reduce salt') || directive.includes('reduce sodium')) {
              cpdDietaryTerms.push('low-sodium');
              userDietaryRestrictions['low-sodium'] = true;
            }
            if (directive.includes('high protein') || directive.includes('increase protein')) {
              cpdDietaryTerms.push('high-protein');
              userDietaryRestrictions['high-protein'] = true;
            }
            if (directive.includes('high fiber') || directive.includes('increase fiber')) {
              cpdDietaryTerms.push('high-fiber');
              userDietaryRestrictions['high-fiber'] = true;
            }
            if (directive.includes('mediterranean')) {
              cpdDietaryTerms.push('mediterranean');
              userDietaryRestrictions['mediterranean'] = true;
            }
            if (directive.includes('vegetarian')) {
              cpdDietaryTerms.push('vegetarian');
              userDietaryRestrictions['vegetarian'] = true;
            }
            if (directive.includes('vegan')) {
              cpdDietaryTerms.push('vegan');
              userDietaryRestrictions['vegan'] = true;
            }
            if (directive.includes('keto') || directive.includes('ketogenic')) {
              cpdDietaryTerms.push('keto');
              userDietaryRestrictions['keto'] = true;
            }
            if (directive.includes('paleo')) {
              cpdDietaryTerms.push('paleo');
              userDietaryRestrictions['paleo'] = true;
            }
            if (directive.includes('whole grain')) {
              cpdDietaryTerms.push('whole-grain');
              userDietaryRestrictions['whole-grain'] = true;
            }
            if (directive.includes('reduce sugar') || directive.includes('less sugar')) {
              cpdDietaryTerms.push('low-sugar');
              userDietaryRestrictions['low-sugar'] = true;
            }
            
            // Detect allergies from CPDs
            if (directive.includes('allergy') || directive.includes('allergic')) {
              if (directive.includes('nut') || directive.includes('peanut')) {
                userAllergies.push('nuts');
              }
              if (directive.includes('dairy') || directive.includes('milk') || directive.includes('lactose')) {
                userAllergies.push('dairy');
              }
              if (directive.includes('gluten') || directive.includes('wheat')) {
                userAllergies.push('gluten');
              }
              if (directive.includes('shellfish')) {
                userAllergies.push('shellfish');
              }
              if (directive.includes('soy')) {
                userAllergies.push('soy');
              }
              if (directive.includes('egg')) {
                userAllergies.push('eggs');
              }
            }
          }
        }
      ...
      // Add CPD-derived terms to dietary preferences with uniqueness
      if (cpdDietaryTerms.length > 0) {
        dietaryPreferences.push(...cpdDietaryTerms);
      }
      
      // Add user allergies from form to our tracked allergies
      if (req.body.allergies && typeof req.body.allergies === 'object') {
        Object.entries(req.body.allergies).forEach(([allergy, isChecked]) => {
          if (isChecked && !userAllergies.includes(allergy)) {
            userAllergies.push(allergy);
          }
        });
      }
      
      // Create final filters with unique dietary preferences
      const filters = {
        ingredients: Array.isArray(req.body.ingredients) ? req.body.ingredients : [],
        cuisineType: req.body.cuisineType,
        mealType: req.body.mealType,
        dietaryPreferences: Array.from(new Set(dietaryPreferences)), // Ensure uniqueness
  ...
  app.post("/api/admin/doctors", async (req, res) => {
    try {
      ...
      // Generate UIN for the doctor
      const uin = generateUIN('DOC');
  ...
  app.post("/api/admin/patients", async (req, res) => {
    try {
      ...
      // Generate UIN for the patient
      // Get the doctorId from the request body to create patient with doctor's initials in UIN
      const doctorId = req.body.doctorId;
      
      // Generate UIN
      const uin = generateUIN('PAT', doctorId);
  ...
  app.post("/api/doctor/patients/:patientId/reports", async (req, res) => {
    try {
      const patientId = parseInt(req.params.patientId);
      const doctorId = req.body.doctorId || 3; // Default to doctor ID 3 for now
  ...
  app.patch('/api/progress-milestones/:id', async (req, res) => {
    try {
      ...
      const existingMilestone = await db
        .select()
        .from(schema.progressMilestones)
        .where(eq(schema.progressMilestones.id, id))
        .limit(1);
      ...
      const updateData: Partial<typeof schema.progressMilestones.$inferInsert> = {
        updatedAt: new Date()
      };
      ...
      // Update in database
      const [updatedMilestone] = await db
        .update(schema.progressMilestones)
        .set(updateData)
        .where(eq(schema.progressMilestones.id, id))
        .returning();
  ...
  app.delete('/api/progress-milestones/:id', async (req, res) => {
    try {
      ...
      await db
        .delete(schema.progressMilestones)
        .where(eq(schema.progressMilestones.id, id));
          ...
          if (existingMilestone) {
            ...
            if (localUpdatedAt > serverUpdatedAt) {
              ...
              const [updatedMilestone] = await db
                .update(schema.progressMilestones)
                .set(updateData)
                .where(eq(schema.progressMilestones.id, localMilestone.id))
                .returning();
        ...
        if (localMilestone.localUuid) {
          ...
          if (similarMilestone) {
            ...
            const [updatedMilestone] = await db
              .update(schema.progressMilestones)
              .set(updateData)
              .where(eq(schema.progressMilestones.id, similarMilestone.id))
              .returning();
            ...
            const [createdMilestone] = await db
              .insert(schema.progressMilestones)
              .values(newMilestone)
              .returning();
  ...
  app.post("/api/patient/login/send-sms", async (req, res) => {
    try {
      ...
      // Store code temporarily
      patientSmsVerificationCodes.set(email, { code: smsCode, expiresAt });
  ...
  app.post("/api/patient/login/verify-sms", async (req, res) => {
    try {
      ...
      const storedCode = patientSmsVerificationCodes.get(email);
