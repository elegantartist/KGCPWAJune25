...
const DietLogistics: React.FC = () => {
  ...
  const userId = userData?.id || 1; // Default to 1 if not found
  ...
  // Fetch the user's active care plan directives
  const { data: directives, isLoading: isLoadingDirectives } = useQuery<CarePlanDirective[]>({
    queryKey: [`/api/users/${userId}/care-plan-directives/active`],
  });
  
  // Find diet related care plan directive
  const dietDirective = directives?.find(d => d.category === 'diet');
          ...
          <CardContent className="pt-4">
            {isLoadingDirectives ? (
              ...
              <div className="p-2 border-l-4 border-primary bg-primary/5">
                <p className="text-gray-700">{dietDirective.directive}</p>
              </div>
            ) : (
