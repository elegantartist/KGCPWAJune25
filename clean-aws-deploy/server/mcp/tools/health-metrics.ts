...
async function getLatestHealthMetrics(userId: number, context: MCPContext) {
  ...
  const historicalMetrics = await storage.getHealthMetricsForUser(userId, 30); // Last 30 days
  ...
  return {
    currentMetrics: {
      ...
      submittedAt: latestMetrics.createdAt
    },
    averages,
    streaks,
    insights,
...
async function getHistoricalHealthMetrics(userId: number, startDate?: string, endDate?: string, context?: MCPContext) {
  ...
  const metrics = await storage.getHealthMetricsInDateRange(userId, start, end);
