...
export default function DoctorDashboard() {
  ...
  const { 
    ...
    queryKey: ["/api/doctor/reports", isViewingReport],
    queryFn: async () => {
      const res = await fetch(`/api/doctor/reports/${isViewingReport}`);
      if (!res.ok) throw new Error('Failed to fetch report details');
      return res.json();
    },
    enabled: isViewingReport !== null,
    retry: false,
  });
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">Manage your patients and their care plans</p>
        </div>
                        ...
                        <Card className="h-fit">
                          <CardHeader className="p-3 pb-2">
                            ...
                            <CardDescription className="text-xs">
                              {patientDetails?.carePlanDirectives?.find((cpd: any) => cpd.category === 'diet')?.updatedAt ? 
                                `Updated: ${format(new Date(patientDetails?.carePlanDirectives?.find((cpd: any) => cpd.category === 'diet')?.updatedAt), "MMM d")}` :
                                "Not set"}
                            </CardDescription>
                          </CardHeader>
                    ...
                    {isLoadingReports ? (
                      ...
                      isLoadingReportDetails ? (
                        ...
                        <div className="space-y-4">
                          <div className="flex justify-between items-center">
                            <div>
                              ...
                              <p className="text-sm text-gray-500">
                                Period: {format(new Date(reportDetails?.reportPeriodStartDate), "MMM d")} - {format(new Date(reportDetails?.reportPeriodEndDate), "MMM d, yyyy")}
                              </p>
                            </div>
                            <Button 
                              variant="outline" 
                          ...
                          {/* New CPD Suggestions */}
                          {reportDetails?.newCpdSuggestions && (
                            <div className="border rounded-md p-4">
                              <h4 className="text-md font-medium mb-3 flex items-center">
                                <Lightbulb className="h-5 w-5 mr-2 text-[#2E8BC0]" />
                                Suggested CPD Updates
                              </h4>
                              <div className="space-y-3">
                                {reportDetails.newCpdSuggestions.diet && (
                                  <Card className="bg-gray-50">
                                    <CardHeader className="pb-2">
                                      <div className="flex items-center">
                                        <Utensils className="h-4 w-4 mr-2 text-[#2E8BC0]" />
                                        <CardTitle className="text-sm font-medium">Diet Plan Update</CardTitle>
                                      </div>
                                    </CardHeader>
                                    <CardContent>
                                      <p className="text-sm">{reportDetails.newCpdSuggestions.diet}</p>
                                    </CardContent>
                                  </Card>
                                )}

                                {reportDetails.newCpdSuggestions.exercise && (
                                  <Card className="bg-gray-50">
                                    <CardHeader className="pb-2">
                                      <div className="flex items-center">
                                        <Activity className="h-4 w-4 mr-2 text-[#2E8BC0]" />
                                        <CardTitle className="text-sm font-medium">Exercise Plan Update</CardTitle>
                                      </div>
                                    </CardHeader>
                                    <CardContent>
                                      <p className="text-sm">{reportDetails.newCpdSuggestions.exercise}</p>
                                    </CardContent>
                                  </Card>
                                )}

                                {reportDetails.newCpdSuggestions.medication && (
                                  <Card className="bg-gray-50">
                                    <CardHeader className="pb-2">
                                      <div className="flex items-center">
                                        <Pill className="h-4 w-4 mr-2 text-[#2E8BC0]" />
                                        <CardTitle className="text-sm font-medium">Medication Plan Update</CardTitle>
                                      </div>
                                    </CardHeader>
                                    <CardContent>
                                      <p className="text-sm">{reportDetails.newCpdSuggestions.medication}</p>
                                    </CardContent>
                                  </Card>
                                )}
                              </div>
                        ...
                        {patientReports.map((report: any) => (
                          <Card key={report.id} className="bg-gray-50 hover:bg-gray-100 cursor-pointer"
                            ...
                            <CardContent className="p-4">
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="font-medium">{format(new Date(report.reportDate), "MMMM d, yyyy")}</p>
                                  <p className="text-sm text-gray-500">
                                    Period: {format(new Date(report.reportPeriodStartDate), "MMM d")} - {format(new Date(report.reportPeriodEndDate), "MMM d, yyyy")}
                                  </p>
                                </div>
                                <div className="flex items-center">
