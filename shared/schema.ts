...
// Main users table - now extended for multiple user types
export const users = pgTable("users", {
  id: serial("id").primaryKey(),
  uin: varchar("uin", { length: 15 }).unique(), // Unique Identity Number for users (format: XXX-1234567)
...
// Table to track doctor-patient relationships
export const doctorPatients = pgTable("doctor_patients", {
  id: serial("id").primaryKey(),
  doctorId: integer("doctor_id").notNull().references(() => users.id),
  patientId: integer("patient_id").notNull().references(() => users.id),
  assignedDate: timestamp("assigned_date").defaultNow().notNull(),
  active: boolean("active").default(true).notNull(), // This maps to 'active' in the database
  notes: text("notes"),
  lastReviewed: timestamp("last_reviewed"),
}, (table) => {
  // Ensure a patient can only be assigned to a doctor once (when active)
  return {
