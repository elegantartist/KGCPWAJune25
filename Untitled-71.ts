...
doctorRouter.get('/patients', authMiddleware(['doctor']), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    logger.error("Error fetching doctor's patients", { doctorId: req.user?.id, error: error instanceof Error ? error.message : String(error) });
...
doctorRouter.post('/create-patient', authMiddleware(['doctor']), async (req: AuthenticatedRequest, res) => {
    try {
        ...
        logger.error("Error creating patient from doctor dashboard", { doctorId: req.user?.id, error: error instanceof Error ? error.message : String(error) });
...
doctorRouter.post('/setup/validate-token', async (req, res) => {
    try {
        ...
        logger.error("Error validating setup token", { token: req.body.token, error: error instanceof Error ? error.message : String(error) });
...
doctorRouter.post('/setup/send-verification', async (req, res) => {
    try {
        ...
        if (error instanceof z.ZodError) return res.status(400).json({ error: 'Invalid request format.' });
        logger.error("Error sending verification code", { token: req.body.token, error: error instanceof Error ? error.message : String(error) });
        res.status(500).json({ error: 'Internal server error.' });
    }
});
...
doctorRouter.post('/setup/verify-phone', async (req, res) => {
    try {
        ...
        if (error instanceof z.ZodError) return res.status(400).json({ error: 'Invalid request format.' });
        logger.error("Error verifying phone code", { token: req.body.token, error: error instanceof Error ? error.message : String(error) });
        res.status(500).json({ error: 'Internal server error.' });
    }
});
...
doctorRouter.post('/setup/complete', async (req, res) => {
    try {
        ...
        logger.error("Error completing doctor setup", { token: req.body.token, error: error instanceof Error ? error.message : String(error) });
        res.status(500).json({ error: 'Internal server error.' });
    }
});
